...
##############################
# Overall test and ratios accuracy
##############################
master_df_goup_by_without_test = master %>% group_by(Task, Train, Congruity) %>%
                   summarise(Accuracy = mean(Validation.Accuracy),
                             std = sd(Validation.Accuracy),
                             .groups = 'drop')

group_by_df_overall_test_df <- subset(master_df_goup_by_without_test, (Task == 'physical'))
p_overall_test_ratios <- ggplot(group_by_df_overall_test_df, aes(x=Train, y=Accuracy, fill=Congruity)) +
  theme_classic2() +  scale_fill_manual(values = c("#880C99", "#CA98D1"))+ #scale_fill_brewer(palette="Set1") +
  geom_bar(stat="identity",  colour = "black", position=position_dodge()) +
  geom_errorbar( aes(x=Train, ymin=Accuracy-std, ymax=Accuracy+std),
                 width=0.3, colour="black", alpha=0.5, size=0.5, position=position_dodge(.9)) +
  ggtitle("Physical-Numerical Task Overall Test and Ratios") +  theme(plot.title = element_text(hjust = 0.5)) +
  theme(text = element_text(size = 20, family="Times New Roman", face="bold"))
p_overall_test_ratios

#############
# Size
#############
size_df <- subset(master_df_goup_by, (Task == 'physical'))
...
p_size <- ggplot(size_df, aes(x=Ratio, y=Accuracy, fill=Congruity)) +
  theme_classic2() +  scale_fill_manual(values = c("#056F85", "#7CC1CF"))+ #scale_fill_brewer(palette="Set1") +
  geom_bar(stat="identity",  colour = "black", position=position_dodge()) +
  geom_errorbar( aes(x=Ratio, ymin=Accuracy-std, ymax=Accuracy+std),
                 width=0.3, colour="black", alpha=0.5, size=0.5, position=position_dodge(.9)) +
  ggtitle("Physical Task") +  theme(plot.title = element_text(hjust = 0.5)) +
  theme(text = element_text(size = 20, family="Times New Roman", face="bold"))
...
# Count-size
#############
count_size_df <- subset(master_df_goup_by, (Task == 'numerical-physical'))
...
p_count_size <- ggplot(count_size_df, aes(x=Ratio, y=Accuracy, fill=Congruity)) +
  theme_classic2() +  scale_fill_manual(values = c("#6C0475", "#C584CB"))+ #scale_fill_brewer(palette="Set1") +
  geom_bar(stat="identity",  colour = "black", position=position_dodge()) +
  geom_errorbar( aes(x=Ratio, ymin=Accuracy-std, ymax=Accuracy+std),
                 width=0.3, colour="black", alpha=0.5, size=0.5, position=position_dodge(.9)) +
  ggtitle("Numerical-physical Task") +  theme(plot.title = element_text(hjust = 0.5)) +
  theme(text = element_text(size = 20, family="Times New Roman", face="bold"))
...
# Color-count
#############
colors_count_df <- subset(master_df_goup_by, (Task == 'colors'))
...
p_control_compare_with_ratio_df <- ggplot(control_compare_group_by_with_ratio, aes(x=Ratio, y=Accuracy, fill=Congruity)) +
  theme_classic2() +  scale_fill_manual(values = c("#4C83CA", "#9CB8DC")) +
  geom_bar(stat="identity",  colour = "black", position=position_dodge()) +
  geom_errorbar( aes(x=Ratio, ymin=Accuracy-std, ymax=Accuracy+std),
                 width=0.3, colour="black", alpha=0.5, size=0.5, position=position_dodge(.9)) +
  ggtitle("Physical Tasks Performance") +  theme(plot.title = element_text(hjust = 0.5)) +
  theme(text = element_text(size = 20, family="Times New Roman", face="bold"))

p_control_compare_with_ratio_df + facet_grid(Train ~ Task)

#difference without ratio
...
control_compare_group_by$Task <- factor(control_compare_group_by$Task, levels = c("physical", "numerical-physical", "colors"),
                  labels = c("phys", "num-phys", "colors")
                  )
p_control_compare_df <- ggplot(control_compare_group_by, aes(x=Task, y=Accuracy, fill=Congruity)) +
  theme_classic2() +  scale_fill_manual(values = c("#6A0378", "#C895CF"))+ #scale_fill_brewer(palette="Set1") +
  geom_bar(stat="identity",  colour = "black", position=position_dodge()) +
  geom_errorbar( aes(x=Task, ymin=Accuracy-std, ymax=Accuracy+std),
                 width=0.3, colour="black", alpha=0.5, size=0.5, position=position_dodge(.9)) +
