...
master <- read.csv("/Users/gali.k/phd/phd_2021/data_analysis/analysis/simulaiton_6/simulation_6_anova_df_for_R_15_01_23_numerical_accuracy.csv", header = TRUE,
...
p_control_compare_df <- ggplot(control_compare_group_by, aes(x=Task, y=Accuracy, fill=Congruity)) +
  theme_classic2() +  scale_fill_manual(values = c("#CD7906", "#F7BA68"))+ #scale_fill_brewer(palette="Set1") +
  geom_bar(stat="identity",  colour = "black", position=position_dodge()) +
  geom_errorbar( aes(x=Task, ymin=Accuracy-std, ymax=Accuracy+std),
                 width=0.3, colour="black", alpha=0.5, size=0.5, position=position_dodge(.9)) +
