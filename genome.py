...
class Genome():
    ...
    def train(self, trainingset, mode, path, epochs, debug_mode, mode_th):
        ...
        if self.accuracy == 0.0: #don't bother retraining ones we already trained 
            self.accuracy, self.train_acc, self.train_loss, self.accuracy_list, self.val_loss = train_and_score(self, trainingset, mode, path, epochs, debug_mode, mode_th, self.accuracy, self.val_loss)
