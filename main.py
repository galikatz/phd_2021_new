...
def generate(generations, generation_index, population, all_possible_genes, dataset, mode, mode_th, images_dir,
	...
	concat_dataframes_into_raw_data_csv_cross_generations(dataframe_list_of_results, "Results_%s_[Mode:%s_Generations:%s_Population:_%s_Epochs:%s_AvgAccuracy:%.2f%%_Time:%s_minutes].csv" % (now_str, mode, str(i), str(population), str(epochs), avg_accuracy, str(round(total_time, 3))))
